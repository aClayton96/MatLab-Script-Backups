...
% Modified from Binary Search Test Harness, Author: Ian van der Linde
% Rev. Date: 08-02-18
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for N = 1:maxArraySize    
    ...
    for searchTarget = 1:N       
        comparisons(searchTarget) = ternarySearchProto(array, searchTarget);        
    end % end for
...
avg = floor(log3((1:maxArraySize)+1)-1);
worst = floor(log3((1:maxArraySize)+1));

plot([1:maxArraySize], linspace(1,1,maxArraySize),        'k:', 'LineWidth', 2); % best
plot([1:maxArraySize], avg, 'k:', 'LineWidth', 2); % average
plot([1:maxArraySize], worst,     'k:', 'LineWidth', 2); % worst
...
print -f1 -r300 -dbmp TernarySearchSucessful.bmp
